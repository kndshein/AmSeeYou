...
const MoviesList = (props) => {
  ...
  const [trailersListState, setTrailersListState] = React.useState(null);
  ...
  const getAllTrailers = () => {
    const arrayOfPromises = props.moviesList.map(async (movie) => {
      const response = await fetch(
        `https://api.themoviedb.org/3/${movie.type}/${movie.id}/videos?api_key=${REACT_APP_APIKEY}&language=en-US`
      );
      const data = await response.json();
      return data; // each loop will return the data in the new array
    });
  ...
  React.useEffect(() => {
    ...
    Promise.all(getAllTrailers()).then((arrayOfData) =>
      setTrailersListState(arrayOfData)
    );
  }, []);
  ...
  return (
    <div className="movies-list">
      <SingleMovie
        ...
        trailersListState={trailersListState}
